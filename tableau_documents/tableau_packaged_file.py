...
class TableauPackagedFile(TableauBase):
    def __init__(self, zip_file_obj, logger_obj=None):
        ...
        self.log('TableauPackagedFile initializing')
        ...
        for name in self.zf.namelist():
            ...
            if name.find('/') == -1:
                if name.endswith('.tds'):
                    self.log('Detected a .TDS file in archive, creating a TableauDatasource object')
                    self.type = 'tdsx'
                    self.xml_name = name
                    ...
                    self.log('Detected a .TWB file in archive, creating a TableauDatasource object')
                    self.type = 'twbx'
                    self.xml_name = name
