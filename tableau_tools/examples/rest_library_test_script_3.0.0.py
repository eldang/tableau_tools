...
def run_tests(server_url, username, password):
    ...
    groups_luids = list(groups_dict.values())
...
def project_permissions_tests(t, project_luid, group_luids):
    ...
    for group_luid in group_luids:
        gcap = t.get_grantee_capabilities_object('group', group_luid, content_type='project')
        if t.api_version == "2.0":
            ...
            gcap.set_capability("View", "Allow")
            gcap.set_capability("Save", "Allow")

            wb_def_gcap = t.get_grantee_capabilities_object('group', group_luid, content_type='workbook')
            wb_def_gcap.set_capabilities_to_match_role("Interactor")
